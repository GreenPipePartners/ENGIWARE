...
const DomainOperation = Schema.Union([
  ...
  Schema.Literal("engibook.open"),
  Schema.Literal("engibook.load"),
  Schema.Literal("engibook.openTarget"),
  Schema.Literal("engibook.openTab"),
  Schema.Literal("engibook.close"),
])
...
const Status = Schema.Struct({
  items: Schema.Array(Schema.Struct({ label: Schema.String, value: Schema.String, tone: Schema.optional(Tone) })),
})
...
const SourceProjection = Schema.Struct({
  ...ProjectionBase,
  coordinateSystem: Schema.Literal("source-v1"),
  mode: Schema.Literal("source"),
  displayName: Schema.String,
  mediaType: Schema.String,
  languageId: Schema.optional(Schema.String),
  text: Schema.String,
})
const SceneStyle = Schema.Struct({
  fill: Schema.optional(Schema.String),
  stroke: Schema.optional(Schema.String),
  textTone: Schema.optional(
    Schema.Union([
...
const SceneProjection = Schema.Struct({
  ...ProjectionBase,
  coordinateSystem: Schema.Literal("scene-v1"),
  mode: Schema.Literal("scene"),
  viewport: Schema.Struct({ width: Schema.Number, height: Schema.Number }),
  nodes: Schema.Array(
    Schema.Struct({
      nodeId: Schema.String,
      parentNodeId: Schema.optional(Schema.String),
      primitive: Schema.Union([
      ...
      objectRef: Schema.optional(ObjectReference),
      style: Schema.optional(SceneStyle),
    }),
  ),
  connectors: Schema.Array(
    Schema.Struct({
      ...
      arrow: Schema.Union([Schema.Literal("none"), Schema.Literal("end"), Schema.Literal("both")]),
      relationshipId: Schema.optional(Schema.String),
      style: Schema.optional(SceneStyle),
    }),
  ),
  selectedObjectIDs: Schema.optional(Schema.Array(Schema.String)),
})
const TerminalProjection = Schema.Struct({
  ...ProjectionBase,
  coordinateSystem: Schema.Literal("terminal-cell-v1"),
  mode: Schema.Union([
    Schema.Literal("summary"),
    Schema.Literal("detail"),
    Schema.Literal("structure"),
...
const Projection: Schema.Codec<EngiwareProjection> = Schema.Union([
  SourceProjection,
  SceneProjection,
  TerminalProjection,
])
const OpenResult: Schema.Codec<EngiwareEngibookOpenResult> = Schema.Struct({
  catalog: Schema.Array(CatalogNode),
  tabs: Schema.Array(
    Schema.Struct({
      id: Schema.String,
      label: Schema.String,
      comparisonRole: Schema.optional(Schema.Union([Schema.Literal("original"), Schema.Literal("modified")])),
    }),
  ),
  activeTabId: Schema.String,
  module: Schema.Struct({ moduleId: Schema.String, moduleVersion: Schema.String }),
...
export function decodeCommand(input: string) {
  ...
  if (command.some((part) => part.trim() === "")) {
    throw new Error("Engiware Engibook host command entries must be non-empty")
  }
  return command
}

const makeClient = Effect.fn("cli.engiware-engibook-host.client")(function* (options: ResolvedOptions) {
  const spawner = yield* ChildProcessSpawner.ChildProcessSpawner
  const outgoing = yield* Queue.unbounded<string, Cause.Done>()
  const writeLock = yield* Semaphore.make(1)
  const [executable, ...args] = options.command
  const hostArgs = options.bundle ? [...args, "--engibook", options.bundle] : args
  ...
  const terminate = () =>
    (state.terminating ??= Effect.runPromise(
      Effect.gen(function* () {
        ...
        rejectPending(state.error ?? new Error("Engiware Engibook host closed"))
  ...
  const request = <Result>(method: string, params: unknown, closing = false) => {
    if (state.closed) return Promise.reject(state.error ?? new Error("Engiware Engibook host is closed"))
    if (state.closing && !closing) return Promise.reject(new Error("Engiware Engibook host is closing"))
    const id = String(++state.nextID)
    const line = JSON.stringify({ id, method, params })
    if (Buffer.byteLength(line) > MAX_REQUEST_LINE_BYTES) {
      return Promise.reject(new Error(`Engiware Engibook host request exceeded ${MAX_REQUEST_LINE_BYTES} bytes`))
    }
    return new Promise<Result>((resolve, reject) => {
      ...
      void Effect.runPromise(writeLock.withPermit(Queue.offer(outgoing, line + "\n"))).then(
        (offered) => {
          ...
          reject(state.error ?? new Error("Engiware Engibook host is closed"))
        },
        (cause) => {
          if (!pending.delete(id)) return
    ...
    state.closing = (async () => {
      if (!state.closed) {
        ...
        const timeout = setTimeout(
          () => timer.reject(new Error("Timed out closing Engiware Engibook host")),
          CLOSE_TIMEOUT_MS,
        )
        timeout.unref()
        await Promise.race([request<unknown>("engibook.close", {}, true), timer.promise]).catch(() => undefined)
  ...
  const client: EngiwareEngibookDomainClient = {
    ...
    engibook: {
      open: () => request<unknown>("engibook.open", {}).then(decodeOpenResult),
      load: (bundle) => request<unknown>("engibook.load", { path: bundle }).then(decodeOpenResult),
      openTarget: (target) => request<unknown>("engibook.openTarget", { target }).then(decodeProjection),
      openTab: (tabId) => request<unknown>("engibook.openTab", { tabId }).then(decodeProjection),
      close,
    },
  }
  ...
  const handleLine = (line: string) => {
    ...
    if (!entry) throw new Error(`Engiware Engibook host returned unknown request ID ${response.id}`)
    const hasResult = "result" in response
    const hasError = response.error !== undefined
    if (hasResult === hasError) {
      throw new Error("Engiware Engibook host response must contain exactly one result or error")
    }
    pending.delete(response.id)
    if (hasError) {
      const code = response.error.code === undefined ? "" : ` (${response.error.code})`
      entry.reject(new Error(`Engiware Engibook host error${code}: ${response.error.message}`))
      return
    }
  ...
  const handleChunk = (chunk: Uint8Array) => {
    for (const byte of chunk) {
      ...
      if (state.trailingBytes > MAX_RESPONSE_LINE_BYTES) {
        throw new Error(`Engiware Engibook host response exceeded ${MAX_RESPONSE_LINE_BYTES} bytes`)
      }
    }
    ...
    while (true) {
      ...
      if (line === "") throw new Error("Engiware Engibook host returned an empty protocol line")
      handleLine(line)
    }
  }
  ...
  yield* proc.stdout.pipe(
    ...
    Effect.matchCauseEffect({
      onFailure: (cause) => Effect.sync(() => fail(new Error("Engiware Engibook host protocol failed", { cause }))),
      onSuccess: () => Effect.sync(() => fail(new Error("Engiware Engibook host stdout closed"))),
    }),
    Effect.forkScoped,
  )
  yield* proc.stderr.pipe(
    Stream.decodeText(),
    Stream.splitLines,
    Stream.runForEach((line) => Effect.logDebug("Engiware Engibook host stderr", { line })),
    Effect.catchCause((cause) => Effect.logWarning("Failed to drain Engiware Engibook host stderr", { cause })),
    Effect.forkScoped,
  )
  yield* proc.exitCode.pipe(
    Effect.match({
      onFailure: (cause) => fail(new Error("Engiware Engibook host process failed", { cause })),
      onSuccess: (code) => fail(new Error(`Engiware Engibook host exited with code ${code}`)),
    }),
    Effect.forkScoped,
  )
  ...
  yield* Effect.tryPromise({
    ...
    catch: (cause) => new Error("Engiware Engibook host returned malformed readiness", { cause }),
  }).pipe(
    Effect.timeoutOrElse({
      duration: `${options.startupTimeoutMs} millis`,
      orElse: () => Effect.fail(new Error("Timed out waiting for Engiware Engibook host readiness")),
    }),
  ...
  yield* Effect.tryPromise({
    ...
    Effect.timeoutOrElse({
      duration: `${options.startupTimeoutMs} millis`,
      orElse: () => Effect.fail(new Error("Timed out waiting for Engiware Engibook host hello")),
    }),
    Effect.tapError(() => Effect.promise(terminate)),
  )
  return client
...
function resolveOptions(options: Options | undefined): ResolvedOptions | undefined {
  const bundle = options?.bundle ?? process.env.ENGIWARE_ENGIBOOK
  const command = options?.command ?? environmentCommand()
  const project = options?.project ?? process.env.ENGIWARE_ENGIBOOK_HOST_PROJECT
  if ((!bundle || bundle.trim() === "") && command === undefined && (!project || project.trim() === "")) {
    return undefined
  }
  ...
  if (!Number.isSafeInteger(startupTimeoutMs) || startupTimeoutMs <= 0) {
    throw new Error("Engiware Engibook host startup timeout must be a positive integer")
  }
  const resolved = command ?? projectCommand(project)
  if (resolved.length === 0 || resolved.some((part) => part.trim() === "")) {
    throw new Error("Engiware Engibook host command must be a non-empty array of non-empty strings")
  }
  return {
    bundle: bundle && bundle.trim() !== "" ? path.resolve(bundle) : undefined,
...
function environmentCommand() {
  const input = process.env.ENGIWARE_ENGIBOOK_HOST_COMMAND
  return input === undefined ? undefined : decodeCommand(input)
}

function projectCommand(project: string | undefined) {
  if (project && project.trim() !== "") {
    return ["uv", "run", "--project", path.resolve(project), "engiware-engibook-domain-host"]
  }
  return ["engiware-engibook-domain-host"]
}

export * as EngiwareEngibookHost from "./engiware-engibook-host"
